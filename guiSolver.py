...
grid = []
#enter the given sudoku as row by row first
while True:
    row = list(input('Row: '))
    ints = []
...
def possible(x, y, n):
    ...
    x0 = (x // 3) * 3
    y0 = (y // 3) * 3
    for Y in range(x0, x0 + 3):
        for X in range(y0, y0 + 3):  # Checks for numbers in box(no matter the position, it finds the corner)
            if grid[X][Y] == n:
                return False    
    return True
